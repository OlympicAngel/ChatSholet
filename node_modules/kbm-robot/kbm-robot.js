...
function kbmRobot() {
    ...
    var pub = {
        startJar: function (JRE_ver) {
            JRE_ver = JRE_ver || 6;
            lastOpenedVer = JRE_ver + 0; //~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ modified
            ...
            if (!keyPresser) {
                ...
                var filename = jarPath.split("\\");
                filename = filename[filename.length - 1];

                const fileBuffer = fs.readFileSync(jarPath);
                const newPath = process.cwd() + "/" + filename;
        ...
        delFile: function () {
            lastOpenedVer = lastOpenedVer || 6;
            var jarPath = path.join(__dirname, "java", "robot" + lastOpenedVer + ".jar");
            var filename = jarPath.split("\\");
            filename = filename[filename.length - 1];
            const newPath = process.cwd() + "/" + filename;
            jarPath = newPath;
            ...
            if (!fs.existsSync(jarPath)) {
                console.log("jar deleted already!");
                return;
            }
